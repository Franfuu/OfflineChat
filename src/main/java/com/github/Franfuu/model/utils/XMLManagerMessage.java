...
import java.io.File;
public class XMLManagerMessage {

        public static boolean writeXML(Message ms, String fileName) {
            boolean result = false;
            JAXBContext context;
            try {
                context = JAXBContext.newInstance(ms.getClass());
                Marshaller m = context.createMarshaller();
                m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
                m.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
                m.marshal(ms, new File(fileName));
                result = true;
